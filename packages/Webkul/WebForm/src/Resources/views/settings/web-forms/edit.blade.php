                ...
                <div class="dropdown-list" style="width: 240px">
                    <div class="dropdown-container">
                        <ul>
                            ...
                            <li v-if="create_driver">
                                <span class="">{{ __('web_form::app.driver') }}</span>

                                <ul>
                                    <li
                                        v-for='(attribute, index) in grouped_attributes.drivers'
                                        :data-id="attribute.id"
                                        @click="addAttribute(attribute)"
                                    >
                                        @{{ attribute.name }}
        ...
        Vue.component('web-form-component', {
            ...
            data: function() {
                return {
                    ...
                    create_driver: "{{ (old('create_driver') ?: $webForm->create_driver) ? true : false }}",
            ...
            watch: {
                ...
                create_driver: function(newValue, oldValue) {
                    if (newValue) {
                        return;
                    }

                    this.addedAttributes = this.addedAttributes.filter(attribute => attribute.attribute.entity_type != 'drivers');
                }
            },
