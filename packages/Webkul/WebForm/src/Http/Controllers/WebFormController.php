...
class WebFormController extends Controller
{
    ...
    {
        $tempAttributes = $this->attributeRepository->findWhereIn('entity_type', ['persons', 'leads', 'drivers']);
    ...
    public function store()
    {
        ...
        $data['create_driver'] = isset($data['create_driver']) ? 1 : 0;
    ...
    public function edit($id)
    {
        ...
        $attributes = $this->attributeRepository->findWhere([
            ['entity_type', 'IN', ['persons', 'leads', 'drivers']],
    ...
    {
        if(request('persons.emails.0.value')) {
        ...
        if(request('drivers.emails.0.value')) {
            $driver = $this->driverRepository
                        ->getModel()
                        ->where('email', 'like', "%" . request('drivers.emails.0.value') . "%")
                        ->first();

            if ($driver) {
                request()->request->add(['drivers' => array_merge(request('drivers'), ['id' => $driver->id])]);
            }
        }
        ...
        if ($webForm->create_lead) {
            ...
            if(! empty($driver)) {
                Event::dispatch('contacts.driver.create.before');

                $data = request('drivers');

                request()->request->add(['entity_type' => 'drivers']);

                $data['entity_type'] = 'drivers';

                $driver = $this->driverRepository->create($data);

                Event::dispatch('contacts.driver.create.after', $driver);
            }
        }
